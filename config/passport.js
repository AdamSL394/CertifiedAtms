...
module.exports = (passport) => {
    ...
    passport.deserializeUser((uuid, done) => {
        db.Users.findOne({where: {uuid: uuid}}).then((user) =>{
	        if(user) {
	            done(null, user.get());
	        } else {
    ...
    function(req, email, local_pw, done) {
        ...
        db.Users.findOne({
            ...
            .then((user, err) => {
                ...
                if (user) {
                    return done(null, ('signupMessage', 'That email is already taken.'));
                } else {
    ...
    function(req, email, local_pw, done){ // callback with email and local_pw from our form
        ...
        db.Users.findOne({
            ...
            if (!user.validPassword(req.body.local_pw, user.local_pw)){
                return done(null,  'invalid email / password combination.');
            }
